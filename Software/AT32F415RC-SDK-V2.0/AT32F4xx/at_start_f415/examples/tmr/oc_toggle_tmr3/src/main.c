...
/** @addtogroup 415_TMR_oc_toggle_tmr3 TMR_oc_toggle_tmr3
  * @{
  */
...
int main(void)
{
  ...
  /* tmr3 configuration: output compare toggle mode:
     tmr3clk = system_core_clock,
     the objective is to get tmr3 counter clock at 24 mhz:
     - div = (tmr3clk / tmr3 counter clock) - 1
     cc1 update rate = tmr3 counter clock / ch1_val / 2 = 366.2 hz
     cc2 update rate = tmr3 counter clock / ch2_val / 2 = 732.4 hz
     cc3 update rate = tmr3 counter clock / ch3_val / 2 = 1464.8 hz
     cc4 update rate = tmr3 counter clock / ch4_val / 2 = 2929.6 hz */

  /* compute the div value */
  div_value = (uint16_t)(system_core_clock / 24000000) - 1;

  /* tmre base configuration */
  tmr_base_init(TMR3, 65535, div_value);
  ...
  tmr_channel_value_set(TMR3, TMR_SELECT_CHANNEL_4, ch4_val);
